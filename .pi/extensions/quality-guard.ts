...
const QUALITY_GUARD_REGISTERED = Symbol.for(
  "pi.skills.add-coding-standard.pi-quality-guard.registered",
);
...
export default function piQualityGuardExtension(pi: ExtensionAPI): void {
  ...
  pi.on("session_shutdown", async (event, ctx) => {
    ...
    const message =
      parseHookResult(hook.stdout)?.systemMessage ?? compactMessage(hook.stdout, hook.stderr);
